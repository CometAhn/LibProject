...
<body>
	...
	<main>
				...
				<c:forEach var="i" begin="1" end="<%=total_page%>">
					<ul class="pagination pagination-sm">
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item"><a class="page-link" href="list?pagenum=${i}&items=<%=items%>&text=<%=text %>">${i}</a></li>
							</c:otherwise>
						</c:choose>
						</a>
				</c:forEach>
		...
		<form action="/Lib/list" method="get">
			<input name="pagenum" type="text" value="1" hidden />
			<ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
				<li><select name="items" class="form-select">
						<option value="Title">책 제목</option>>
						<option value="Writer">글쓴이</option>
						<option value="Category">카테고리</option>
						<option value="Publisher">출판사</option>
				</select></li>
				<li><input name="text" class="form-control" type="text" /></li>
				<li><input type="submit" id="btnAdd" class="btn btn-primary" value="검색" /></li>
			</ul>
		</form>
	</main>
